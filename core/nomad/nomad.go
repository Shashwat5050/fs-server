...
func (n *nomadClient) GetNodes(ctx context.Context) ([]models.NodeInfo, error) {
	nodes, _, err := n.client.Nodes().List(&nomadApi.QueryOptions{})
	if err != nil {
		return nil, err
	}

	clients := make([]models.NodeInfo, 0, len(nodes))
	for _, node := range nodes {
		clients = append(clients, models.NodeInfo{
			ID:                node.ID,
...
func (n *nomadClient) GetNodeStats(ctx context.Context, id string) (models.NodeStats, error) {
	node, err := n.client.Nodes().Stats(id, &nomadApi.QueryOptions{})
	if err != nil {
		return models.NodeStats{}, err
	}

	stats := models.NodeStats{
		Uptime:           node.Uptime,
		CPUTicksConsumed: node.CPUTicksConsumed,
	}

	if node.Memory != nil {
		stats.Memory = &models.HostMemoryStats{
			Total:     node.Memory.Total,
			Available: node.Memory.Available,
	...
	if node.CPU != nil {
		for _, cpu := range node.CPU {
			stats.CPU = append(stats.CPU, &models.HostCPUStats{
				CPU:    cpu.CPU,
				User:   cpu.User,
				System: cpu.System,
				Idle:   cpu.Idle,
	...
	if node.DiskStats != nil {
		for _, disk := range node.DiskStats {
			stats.DiskStats = append(stats.DiskStats, &models.HostDiskStats{
				Size:              disk.Size,
				Used:              disk.Used,
